...
async function checkNFCPermission() {
    try {
        ...
        if (nfcPermissionStatus.state === "granted") {
            ...
            document.getElementById('status').textContent = "NFC access is denied. Please enable it in the browser settings.";
        }
        ...
        nfcPermissionStatus.onchange = () => {
            if (nfcPermissionStatus.state === "granted") {
                ...
                console.log(`NFC permission state changed to: ${nfcPermissionStatus.state}`);
            }
        };
    } catch (error) {
